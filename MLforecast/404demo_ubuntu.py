...
def model_train(dsnp, past, future, learning_rate, batch_size, epochs, nlstm, nDeep, train_split, name):
    train_data = dsnp.loc[0:train_split-1][:]
    val_data = dsnp.loc[train_split:][:]

    start = past + future
    end = start + train_split

    x_train = train_data.values
    y_train = dsnp[start:end]["Power"]
    ...
    x_end = val_data.shape[0] - past - future

    label_start = train_split + past + future

    x_val = val_data.iloc[:x_end][:].values
    y_val = dsnp.iloc[label_start:]["Power"]
...
def grid_search_hyperparameters(dsnp, train_split, param_grid, name):
    best_loss = float('inf')
    best_params = None

    for past in param_grid['past']:
        for future in param_grid['future']:
            for learning_rate in param_grid['learning_rate']:
                for batch_size in param_grid['batch_size']:
                    for epochs in param_grid['epochs']:
                        for nlstm in param_grid['nlstm']:
                            for nDeep in param_grid['nDeep']:
                                print(f"Testing hyperparameters: past={past}, future={future}, learning_rate={learning_rate}, batch_size={batch_size}, epochs={epochs}, nlstm={nlstm}, nDeep={nDeep}")

                                # Train model
                                model, history, _ = model_train(dsnp, past, future, learning_rate, batch_size, epochs, nlstm, nDeep, train_split, name)
...
def generate_predictions(model, historical_data, batch_size=24, num_steps=336):
    """
    ...
    # Repeat prediction process for each time step
    for i in range(num_steps):
        # Extract input data for the current time step
        input_data = historical_data.iloc[i:i + batch_size].values.reshape(batch_size, -1, historical_data.shape[1])

        # Predictions for the current time step
        step_predictions = model.predict(input_data)
...
#     print(table.name)
#     if table.name == "ML_forecast":
#         table_list[table.name] =  pd.read_sql_table(table.name, engine)

#hist_data = pd.read_sql_table("hist_data", engine, index_col="Time", parse_dates={"Time": '%Y-%m-%d %H:%M:%S'})
hist_data = pd.read_sql_table("hist_data", engine)
hist_data.index = pd.to_datetime(hist_data.pop("Time"), format='%Y-%m-%d %H:%M:%S', unit='s')
feature_titles = hist_data.columns.tolist()
#timestamp_range = pd.Series(np.arange(hist_data.index.min(), hist_data.index.max(), step=np.timedelta64(1, "h")))
region_titles = ['COAST', 'EAST', 'FWEST', 'NORTH', 'NCENT', 'SOUTH', 'SCENT', 'WEST', 'ERCOT']
regions = {title: hist_data[hist_data['Location'] == title] for title in region_titles}
selected_regions = [region_titles[i] for i in [0, 1, 3, 7]]
selected_features = hist_data.select_dtypes(include='float64').columns.to_numpy(dtype=str).tolist()

selected_data = {i: pd.DataFrame(regions[i][:][selected_features], index=regions[i].index, columns=selected_features) for i in selected_regions}
...
for i in selected_regions:
    #power data
    reg_data = selected_data[i]
    reg_columns = selected_data[i].columns
    reg_timestamps = selected_data[i].index
    reg_data = reg_data.sort_index(axis=0)
    split_fraction = 0.75
    train_split = int(split_fraction * (reg_data.shape[0]-336))
    ...
    pred_data = reg_data.tail(336).sort_index(axis=0)
    forecast_timestamps = pred_data.index + np.timedelta64(14, 'D')
    reg_data = reg_data.sort_index(axis=0)
    reg_data = reg_data.truncate(after=(pred_data.index[0] - np.timedelta64(1, 'h')))
    #reg_columns = reg_columns[:(len(selected_data[i])-336)]
    #reg_timestamps = reg_timestamps[:(len(selected_data[i])-336)]

    #reg_data = selected_data[i].interpolate(method='time', axis=0)


    #envelope_data = pd.DataFrame(data=dict({'amplitude': reg_amp, 'offset': reg_off}), index=regions[i].index, columns=selected_features)
    ts_hr = reg_data
    #general data
    array = ts_hr
    print(array)
    ...
    #best_params = grid_search_hyperparameters(selected_data[i], train_split, param_grid, i)
    #(array, past, future, learning_rate, batch_size, epochs, nlstm, nDeep, train_split)
    if load_model:
        model = keras.saving.load_model(name + "model.keras")
    else:
        model, history, chkpt_callback = model_train(array, past, future, learning_rate, batch_size, epochs, nlstm, nDeep, train_split, name)

    if produce_forecast:
        raw_predict = generate_predictions(model, pred_data, batch_size, future)
        forecast = combFilt(raw_predict, invert=True)
        forecast = denormalize(forecast, reg_mean[-1], reg_std[-1])
        forecast = pd.Series(forecast, index=forecast_timestamps)
        RegionDict[i] = {"model": model,  "mean": reg_mean, "std": reg_std, "ts_hr": ts_hr, "forecast": forecast}
    else:
        #RegionDict[selected_regions[i]] = {"model": model,  "envelope_data": envelope_data, "ts_hr": ts_hr}
        RegionDict[i] = {"model": model,  "mean": reg_mean, "std": reg_std, "ts_hr": ts_hr}
        #RegionDict[i] = {"best_params": best_params,  "mean": reg_mean, "std": reg_std, "ts_hr": ts_hr}
...
# filePath = "ERCOT Data 2002-2023 Fixed Dates.csv"
#
...
# hour_end = df.index
# predict_path = "Native_Load_2024.csv"
...
# for i in range(len(selected_regions)):
#     #power data
#     envelope_data = pd.DataFrame(data=dict({'amplitude': regamp[dsb:dse, i], 'offset': regoffs[dsb:dse, i]}), index=envtimestamps[dsb:dse])
#     #amplitude = np.abs(regamp[-17520:, i])
#     ts_hr = pd.Series(data=regions.to_numpy()[hsb:hse, i], index=hour_end[hsb:hse])
#     power_data = {"envelope_data": envelope_data, "ts_hr": ts_hr}
#     #general data
...
#         model = keras.saving.load_model(name + "model.keras")
#     else:
#         model, history, chkpt_callback = model_train(array, past, future, 0.0025, 48, 10, 64, 12, train_split, name)
#
#     RegionDict[selected_regions[i]] = {"model": model,  "power_data": power_data}
